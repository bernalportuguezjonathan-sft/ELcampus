...
def test_venta_simple_calcula_total_y_descuenta_stock(cliente, datos):
    respuesta = _cobrar(cliente, [{"producto_id": datos["cerveza"].id, "cantidad": 3}])
...
def test_escanear_dos_veces_suma_en_una_sola_linea(cliente, datos):
    respuesta = _cobrar(
        cliente,
        [
...
def test_producto_por_peso_se_cobra_por_kilo(cliente, datos):
    respuesta = _cobrar(cliente, [{"producto_id": datos["morraja"].id, "cantidad": 0.35}])
...
def test_venta_mixta_de_plato_y_producto(cliente, datos):
    respuesta = _cobrar(
        cliente,
        [
...
def test_anular_devuelve_el_stock_y_deja_rastro(cliente, datos):
    venta = _cobrar(cliente, [{"producto_id": datos["cerveza"].id, "cantidad": 5}]).json()
...
def test_no_se_puede_anular_dos_veces(cliente, datos):
    venta = _cobrar(cliente, [{"producto_id": datos["cerveza"].id, "cantidad": 1}]).json()
    cuerpo = {"motivo": "error de digitación"}

    cliente.post(f"/api/ventas/{venta['id']}/anular", json=cuerpo)
    segunda = cliente.post(f"/api/ventas/{venta['id']}/anular", json=cuerpo)

    assert segunda.status_code == 409
    assert datos["cerveza"].stock_actual == 100


def test_venta_vacia_se_rechaza(cliente):
    assert _cobrar(cliente, []).status_code == 400


def test_item_sin_producto_ni_plato_se_rechaza(cliente):
    assert _cobrar(cliente, [{"cantidad": 1}]).status_code == 422


def test_producto_inexistente_no_deja_venta_a_medias(cliente, db, datos):
